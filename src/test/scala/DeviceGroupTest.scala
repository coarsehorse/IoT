...
class DeviceGroupTest(_system: ActorSystem) extends TestKit(_system)
  ...
  "ignore requests for wrong groupId" in {
    val probe = TestProbe()
    val groupActor = system.actorOf(DeviceGroup.props("group1"))
  ...
  "return same actor for same deviceId" in {
    ...
    // create new device
    groupActor.tell(DeviceManager.RequestTrackDevice("group2", "device3"), probe.ref)
    probe.expectMsg(DeviceManager.DeviceRegistered)
    val new_device: ActorRef = probe.lastSender

    // repeat
    groupActor.tell(DeviceManager.RequestTrackDevice("group2", "device3"), probe.ref)
    probe.expectMsg(DeviceManager.DeviceRegistered)

    new_device === probe.lastSender
  }
  ...
  "be able to list active devices" in {
    ...
    probe.expectMsg(DeviceGroup.ReplyDeviceList(requestId = 0, Set("device2", "device1")))
  }
